...
class WC_Gateway_ONVO_Pay extends \WC_Payment_Gateway {
	...
	public function __construct() {
		...
		add_action( 'get_header', [ $this, 'maybe_create_intent' ], 0 );

		// calculate subscription stuff
		add_action( 'get_header', [ $this, 'on_page_change' ] );
  ...
  /**
	 ...
	 * @return void
	 */
	protected function handle_invalid_payment_method_id_on_checkout( int $order_id, $payment_intent_id, $cart_total ): void {
		ONVO\error(
			"Payment method not found for order #$order_id. Payment intent: $payment_intent_id.",
			array(
				'order_id' => $order_id,
				'intent_id' => $payment_intent_id,
			)
		...
		wc_add_notice( __( 'No se han ingresado los datos de pago.', 'onvo' ), 'error' );
		$this->onvo_api->update_intent( \ONVO\Intent_Builder::from_processed_payment_order(
			$payment_intent_id,
			$order_id,
			$this->merchant_key,
			$cart_total
